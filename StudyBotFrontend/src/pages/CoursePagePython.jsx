...
function CoursePagePython() {
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      try {
        // Fetch tasks for Python course (task IDs 21 to 30)
  ...
  return (
    <div className="min-h-screen bg-black text-gray-200 p-8 space-y-8">
      ...
      {tasks.map((task) => (
        <div 
          key={task.task_id} 
          className="bg-black p-6 rounded-md shadow-md border border-white transition-all duration-300 hover:shadow-lg hover:border-yellow-500 space-y-4"
        > 
          <div className="flex justify-between items-center">
            <div>
              <h3 className="text-xl font-semibold text-gray-300">
                Task {task.task_id - 20}: {task.title}
              </h3>
              <p className="text-gray-400 text-sm">
                Watch the video to understand the basics of Python programming.
              </p>
            </div>
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between mt-4 space-y-3 md:space-y-0">
            {/* Watch Video Button */}
            <button
              className="px-4 py-2 bg-black text-gray-200 border border-white rounded-md hover:border-yellow-500 hover:text-yellow-400 transition-all duration-300 hover:shadow-lg text-sm"
            ...
            <div className="flex space-x-3">
              {/* Start Coding Button */}
              <Link to={`/user/:userId/course/python/task/${task.task_id}`}>
                <button className="px-4 py-2 bg-black text-gray-200 border border-white rounded-md hover:border-yellow-500 hover:text-yellow-400 transition-all duration-300 hover:shadow-lg text-sm">
                  Start Coding
                </button>
              </Link>
